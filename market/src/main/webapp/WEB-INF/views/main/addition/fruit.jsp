...
<ul>
	...
	<br></br>
		<c:if test="${pageNumber.p.getPage() > 1}">
			<a href="fruits?page=${pageNumber.p.getPage() - 1}"> << </a>
		</c:if>
		<c:if test="${pageNumber.p.getPage() <= 1}">
			<a href="fruits?page=${pageNumber.p.getPage()}"> << </a>
		</c:if>
		<c:if test="${pageNumber.p.getPage() <= 2 && Math.ceil(pageNumber.getTotal()/3) >= 5}">
			<c:forEach var="idx" begin = "1" end = "5">			    
				<c:if test="${pageNumber.p.getPage() != idx}">
				   <a href="fruits${pageNumber.pages(idx)}">${idx}</a>
				 </c:if>
				 <c:if test="${pageNumber.p.getPage() == idx}">
				   	<a href="fruits${pageNumber.pages(idx)}" style = "color:#c62cd4;">${idx}</a>
				  </c:if>
			</c:forEach>
		</c:if>
			    <c:if test="${pageNumber.p.getPage() <= 2 && Math.ceil(pageNumber.getTotal()/3) < 5}">
			    ...
			    <c:if test="${Math.ceil(pageNumber.getTotal()/3) == 0}">
				    <c:forEach var="idx" begin = "1" end = "1">			    
				    	...
				    	<c:if test="${pageNumber.p.getPage() == idx}">
				    		<a href="fruits${pageNumber.pages(1)}" style = "color:#c62cd4;">1</a>
				    	</c:if>
				    </c:forEach>
			    </c:if>
